...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR1bW15Iiwicm9sZSI6ImFub24iLCJpYXQiOjE2NzE4NTc2NDcsImV4cCI6MTk4NzQzMzY0N30.dummy";
const supabase = createClient(supabaseUrl, supabaseKey);
class SupabaseManager {
  // Stock operations
  static async upsertStock(stock) {
  ...
  static async getActiveOpportunities() {
    const { data, error } = await supabase.from("opportunities").select("*").eq("resolved", false).order("ai_aware_score", { ascending: false }).order("flagged_date", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      symbol: row.symbol,
  ...
  static async updateOpportunity(id, updates) {
    const updateData = {};
    if (updates.resolved !== void 0) updateData.resolved = updates.resolved;
    if (updates.actualReturn !== void 0) updateData.actual_return = updates.actualReturn;
  ...
  static async getUnreadAlerts() {
    const { data, error } = await supabase.from("alerts").select("*").eq("read", false).order("created", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      type: row.type,
  ...
  // Utility functions
  static async getOpportunitiesByType(type) {
    const { data, error } = await supabase.from("opportunities").select("*").eq("type", type).eq("resolved", false).order("ai_aware_score", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      symbol: row.symbol,
  ...
  static async getOpportunitiesByRisk(riskLevel) {
    const { data, error } = await supabase.from("opportunities").select("*").eq("risk_level", riskLevel).eq("resolved", false).order("ai_aware_score", { ascending: false });
    if (error) throw error;
    return (data || []).map((row) => ({
      id: row.id,
      symbol: row.symbol,
  ...
  static async getPerformanceStats() {
    const { data: opportunities, error } = await supabase.from("opportunities").select("*").eq("resolved", true).not("actual_return", "is", null);
    if (error) throw error;
    const stats = {
